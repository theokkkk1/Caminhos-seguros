...
# =========================
# ESTADO DA FASE (nivel de modulo)
# =========================
#
# IMPORTANTE: anteriormente esta fase era executada dentro de um
# "while True" proprio, com seu propio pygame.event.get(). Isso fazia
# com que ela "engolisse" todos os eventos do jogo, incluindo:
#   - a tecla ESC (nao dava pra voltar ao menu durante a Fase 2)
#   - o clique no X da janela (o jogo nao fechava, so caia no menu)
#
# Agora a fase segue o mesmo padrao das Fases 1 e 3: o estado vive
# aqui no modulo e a funcao run_phase2() e chamada uma vez por frame
# pelo loop principal (main.py), que continua sendo o unico lugar
# que le os eventos do pygame.
...
horizontal_road = pygame.Rect(0, 240, WIDTH, 160)

# =========================
# FAIXAS
# =========================

vertical_crosswalk = pygame.Rect(320, 420, 160, 70)
horizontal_crosswalk = pygame.Rect(480, 240, 70, 160)
...
for i in range(3):
    vertical_cars.append(
        pygame.Rect(vertical_lane_x, -250 * i - 120, 45, 80)
    )

for i in range(3):
    horizontal_cars.append(
        pygame.Rect(-320 * i - 200, horizontal_lane_y, 80, 45)
    )
...
# =========================
# RESET (erro do jogador)
# =========================

def reset_game():
    """Reinicio leve, usado quando o jogador erra (atravessa fora da
    faixa ou e atingido por um carro). So devolve o jogador ao inicio
    e zera os itens -- o transito e o semaforo continuam normalmente,
    exatamente como no comportamento original desta fase."""

    global has_vertical, has_horizontal
...
# =========================
# ENTRADA NA FASE (reinicio completo)
# =========================

def enter_phase():
    """Chamado sempre que se entra nesta fase a partir de outra tela.

    Corrige o bug em que jogar a Fase 2 novamente (por exemplo, depois
    de terminar o jogo e voltar ao menu) comecava com os carros e o
    semaforo no estado em que a fase tinha ficado na ultima vez."""

    global has_vertical, has_horizontal, signal_timer, vertical_signal
...
# =========================
# FASE (chamada uma vez por frame)
# =========================
...
def run_phase2(screen):
    ...
    screen.blit(vertical_text, (22, 20))
    screen.blit(horizontal_text, (22, 46))
    ...
    if player.colliderect(school_area):
        ...
        if has_vertical and has_horizontal:
            ...
            victory_text = victory_font.render(
                "VOCE CHEGOU A ESCOLA!",
                True,
                GREEN
            )

            info_text = small_font.render(
                "Parabens por atravessar com seguranca.",
                True,
                WHITE
            )
